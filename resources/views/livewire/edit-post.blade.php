<div>
    ...
    <form method="POST" wire:submit="update">
        <div>
            ...
            <input id="title" wire:model="form.title" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm text-gray-700" type="text" />
            @error('title')
        ...
        <div class="mt-4">
            ...
            <textarea id="body" wire:model="form.body" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm text-gray-700"></textarea>
            @error('body')
            <span class="mt-2 text-sm text-red-600">{{ $message }}</span>
            @enderror
        </div>
